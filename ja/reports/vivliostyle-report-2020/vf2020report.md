...
2019年度、当法人は新たな開発体制を構築した。開発イベントを開催し、そこでコントリビュータになり得るエンジニアを集め、ついで月次で開発者会議を開催し、そこで開発についてコントリビュータと議論するといった体制である。そうした中で立ち上がってきたのが、以下のようなVivliostyle Pubの構想だった。

- プロダクトの概要
    - ブラウザから操作するWebアプリ
    - クラウドにVivliostyle CLIをおき、ユーザのコンテンツはGitHubで管理することにする
    - ユーザはGitHubを意識する必要はないが、ログインのためにGitHubアカウントは取得してもらう
- ターゲットユーザ
    - まずは技術系同人誌をつくるエンジニアをターゲットにする
    - アップデート
- ビジネスモデル
    - 提携する印刷会社に、Vivliostyle Pubを使った専用の印刷プランを用意してもらう
    - その上で、PDFの生成後に提携印刷会社の納品ページに直接遷移できるボタンを設置する
        - これにより、PDFを作ればダイレクトに印刷データの納品が可能になる
    - これにより、印刷会社はVivliostyle Pubを自社の営業窓口にできる
    - 成約した場合、当法人は印刷会社から割り戻しをいただくことにする
    - 複数の印刷会社と提携し、ユーザはその中から自分のニーズに合ったプランを選べるようにする

残された課題はVivliostyle Pubの開発を支える資金の確保であり、これが2020年度の課題となった。

### コロナ禍の襲来

- 4/7：東京、神奈川、埼玉、千葉、大阪、兵庫、福岡の7都府県に緊急事態宣言が出され、空前の景気後退が懸念される状況に突入
- この状況で、当てにしていた外部企業からのVivliostyle Viewer関連の受託開発も棚上げになり、売上の見込みがなくなる
- 当法人は4月に収益事業開始届をだしたばかりなので、国による支援金も当てにできない
- 6月には信用金庫に融資を申し込むが、実績がなく事業の継続性が担保されないとの理由で、あえなく断られてしまう
- ただし業務面に限れば、以前から事務所を持たずにリモートで仕事をするスタイルだったので、とくに影響がなかったのは幸いだった
- 結局、辛かったのは年度前半だけで、巣ごもり需要を追い風にしてIT業界全体が売上を伸ばしたのと同様、後半はむしろコロナ禍が好材料となった（後述）

### 「Vivliostyle ユーザーと開発者の集い 2020春」を開催

- 各種イベントが中止になる中、公式サイトのリニューアル作業で忙殺されていることもあり、イベント自体を中止にする判断もあり得た
- しかし、あえて攻めに出ることにして、オンラインのみに切り替えて開催
- 結果として、参加申込142名という大反響を得ることができた
    - この記録は現在も破られていない
- また発表準備のためにコミットが急増する「イベントドリブン開発」効果も見られた
