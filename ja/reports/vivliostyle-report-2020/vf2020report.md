...
- 新たな開発体制の構築
    - 開発イベントを開催し、そこでコントリビュータになり得るエンジニアを集める
    - 月次で開発者会議を開催することにし、そこで開発についてコントリビュータと議論することにする
- そうして開発者会議からVivliostyle Pubの構想がたちあがる
    - ターゲットユーザ
        - 技術系同人誌をつくるエンジニア
    - 開発の趣旨
        - Vivliostyle CLI をクラウドにおき、コンテンツはGitHubで管理し、ユーザはブラウザで操作するWebアプリ
    - ビジネスモデル
        - 前提として、印刷会社にVivliostyle Pubを使った印刷プランを作ってもらう
        - PDFの生成後に印刷会社別の納品ページに直接遷移できるようにする
        - これにより、PDFを作ればダイレクトに印刷データの納品が可能になる
        - つまり、印刷会社はVivliostyle Pubを自社の営業窓口にできる
        - 成約した場合、当法人は印刷会社から割り戻しをいただくことにする
- 残された課題はVivliostyle Pubの開発を支える資金の確保であり、これが2020年度の課題となる
